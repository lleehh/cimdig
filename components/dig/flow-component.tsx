...
export function colorStyling(CIM: CIM) {
  return (
    <div style={{ backgroundColor: CIM.color?.toString()!, height: "10px" }}>
      {" "}
    </div>
  );
}
...
export default function FlowComponent({ data }: NodeProps<CimNode>) {
  ...
  const { nodes, edges, setNodes, setEdges, setFocusNode } = useFlowStore(
    useShallow(selector),
  );
  ...
  const handleExpand = async () => {
    ...
    const edge = edges.filter(
      (edge) =>
        edge.source === component?.rdfId || edge.target === component?.rdfId,
    );
    ...
    if (node && component) {
      if (isTerminal(component)) {
        if (
          !doesEquipmentExistsInFlow(component.connectivityNode.rdfId, nodes)
        ) {
          newNodes.push(
            createNode(
              component.connectivityNode.rdfId,
              component.connectivityNode,
              0,
              0,
              data.color?.toString()!,
            ),
          ...
          newEdges.push(
            createEdge(component.rdfId, component.connectivityNode.rdfId, true),
          );
        }

        if (
          !doesEquipmentExistsInFlow(component.conductingEquipment.rdfId, nodes)
        ) {
          newNodes.push(
            createNode(
              component.conductingEquipment.rdfId,
              component.conductingEquipment,
              0,
              0,
              data.color?.toString()!,
            ),
      ...
      if (isConnectivityNode(component) || isConductingEquipment(component)) {
        ...
        if (
          terminals.length == 0 &&
          (component as PowerTransformerEnd).terminal != undefined
        )
